...
function createCard(item) {
    ...
    card.innerHTML = `
        <img src="assets/logo-ut.png" class="book-img" onerror="this.style.display='none'">
        <div class="info-box">
...
function tambahData() {
    ...
    if (!nama || !stok) { alert("Isi nama dan stok!"); return; }
    const container = document.getElementById('display-area');
    const newData = {
        namaBarang: nama, stok: stok,
        kodeLokasi: "0TMP01" + (dataBahanAjar.length + 1),
        kodeBarang: "ASIP4301" + (4000 + dataBahanAjar.length),
        jenisBarang: "BMP", edisi: 2
    };
